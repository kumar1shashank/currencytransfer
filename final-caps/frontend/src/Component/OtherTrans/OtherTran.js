...
export default function OtherTran() {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      await axios.post(
        "http://localhost:8080/api/wallettobanktransfer/banktransfer",
        data,
        {
          headers: {
      ...
      }).catch((error) => {console.log(error); setError(error.response.data)});
  ...
  const validationSchema = Yup.object({
    ...
    receiveraccount: Yup.number().required("amount is required"),
    ifsc: Yup.string().required("amount is required"),
